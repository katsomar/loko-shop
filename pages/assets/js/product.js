...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Edit Store Product handler
    const editStoreProductModal = new bootstrap.Modal(document.getElementById('editStoreProductModal'));
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.edit-store-btn')) {
            ...
            // For now, redirect to edit page (similar to shelf products)
            window.location.href = 'edit_product.php?id=' + productId + '&type=store';
        }
    });
    
    // Edit Shelf Product handler
    const editShelfProductModal = new bootstrap.Modal(document.getElementById('editShelfProductModal'));
    
    document.addEventListener('click', function(e) {
        if (e.target.closest('.edit-shelf-btn')) {
            const btn = e.target.closest('.edit-shelf-btn');
            const productId = btn.dataset.id;
            
            // For shelf products, open the edit modal directly
            editShelfProductModal.show();
            
            // Fetch and populate product data
            // Assuming a function fetchShelfProductData exists
            fetchShelfProductData(productId);
        }
    });
    
    document.getElementById('confirmEditShelfProduct')?.addEventListener('click', async function() {
        const productId = document.getElementById('editShelfProductId').value;
        const productName = document.getElementById('editShelfProductName').value;
        const productPrice = document.getElementById('editShelfProductPrice').value;
        const msgDiv = document.getElementById('editShelfProductMessage');
        ...
        try {
            const formData = new FormData();
            formData.append('action', 'edit_shelf_product');
            formData.append('product_id', productId);
