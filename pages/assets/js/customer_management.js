...
// ========================================
// SALES ANALYSIS TAB - Charts & Table
// ========================================
let profitChart = null;
let productChart = null;
// NEW: keep last loaded analysis data for client-side filtering
window.salesAnalysisData = null;

// Load and render analysis data
async function loadSalesAnalysis() {
    const days = document.getElementById('analysisDateRange')?.value || 30;
    const customerId = document.getElementById('analysisCustomer')?.value || 0;
    const productFilter = document.getElementById('analysisProduct')?.value?.trim() || 'all';
    
    const tbody = document.getElementById('analysisDailyOrdersBody');
    if (tbody) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">Loading...</td></tr>';
    }
    
    const params = new URLSearchParams({
        fetch_sales_analysis: '1',
        days: days,
        customer_id: customerId,
    ...
    try {
        ...
        try {
            ...
            if (tbody) {
                tbody.innerHTML = '<tr><td colspan="5" class="text-center text-danger">Error parsing server response</td></tr>';
            }
            return;
        }
        ...
        // NEW: cache for table filters
        window.salesAnalysisData = data;
        
        renderCharts(data);
        // apply current table filters when (re)loading data
        renderDailyOrdersTable(data, getOrdersTableFilters());
    } catch (err) {
...
function renderCharts(data) {
    ...
    const profitTrend = dates.map(d => dailyData[d]?.profit || 0);
    
    // Prepare product breakdown (stacked bar)
    const allProducts = Object.keys(data.product_totals || {});
    const productData = {};
    allProducts.forEach(prod => {
        productData[prod] = dates.map(d => dailyData[d]?.products?.[prod] || 0);
    });
    ...
    // Profit Trend Chart
    const profitCtx = document.getElementById('analysisProfitChart')?.getContext('2d');
    if (profitCtx) {
    ...
    // Product Breakdown Chart (Stacked Bar)
    const colors = [
        '#3498db', '#e74c3c', '#2ecc71', '#f39c12', '#9b59b6',
        '#1abc9c', '#34495e', '#c0392b', '#16a085', '#27ae60'
    ];
    
    const productDatasets = allProducts.map((prod, idx) => ({
        label: prod,
        data: productData[prod],
        backgroundColor: colors[idx % colors.length]
    }));
    
    const productCtx = document.getElementById('analysisProductChart')?.getContext('2d');
    if (productCtx) {
...
// NEW: read current table filter controls
function getOrdersTableFilters() {
    const customerName = document.getElementById('ordersCustomerFilter')?.value.trim() || '';
    const dateFrom = document.getElementById('ordersDateFrom')?.value || '';
    const dateTo = document.getElementById('ordersDateTo')?.value || '';
    return { customerName, dateFrom, dateTo };
}

// Render Daily Orders Table (grouped by date, with optional filters)
function renderDailyOrdersTable(data, filters = {}) {
    const tbody = document.getElementById('analysisDailyOrdersBody');
    if (!tbody) return;
    
    const allOrders = data.order_details || [];
    let orderDetails = allOrders.slice(); // work on copy

    const customerFilter = (filters.customerName || '').toLowerCase();
    const dateFrom = filters.dateFrom || '';
    const dateTo = filters.dateTo || '';

    // Apply customer name filter (substring, case-insensitive)
    if (customerFilter) {
        orderDetails = orderDetails.filter(o =>
            (o.customer_name || '').toLowerCase().includes(customerFilter)
        );
    }

    // Apply date range filter (YYYY-MM-DD strings, same format as o.date)
    if (dateFrom) {
        orderDetails = orderDetails.filter(o => (o.date || '') >= dateFrom);
    }
    if (dateTo) {
        orderDetails = orderDetails.filter(o => (o.date || '') <= dateTo);
    }

    if (orderDetails.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center text-muted">No orders found for the selected filters</td></tr>';
        return;
    }

    // NEW: aggregate duplicates for same day + customer + product
    const aggregatedMap = {};
    orderDetails.forEach(order => {
        const date = order.date || '';
        const customer = order.customer_name || '';
        const product = order.product_name || '';
        const key = `${date}||${customer}||${product}`;
        ...
        aggregatedMap[key].quantity += qty;

        // Update time to earliest (smallest) if multiple rows
        if (order.time && aggregatedMap[key].time) {
            if (order.time < aggregatedMap[key].time) {
                aggregatedMap[key].time = order.time;
            }
        } else if (order.time && !aggregatedMap[key].time) {
            aggregatedMap[key].time = order.time;
        }
    });

    const aggregatedOrders = Object.values(aggregatedMap);

    // Group aggregated orders by date
    const groupedByDate = {};
    ...
    sortedDates.forEach(date => {
        ...
        html += `
            <tr class="table-secondary">
                <td colspan="5" class="fw-bold" style="background: #e9ecef;">
                    📅 ${formattedDate}
                    <span class="badge bg-primary ms-2">${orders.length} order(s)</span>
                </td>
            </tr>
        `;
        
        // Aggregated order rows for this date
        orders.forEach(order => {
            // NOTE: keep prediction based on full history (allOrders), not just filtered/aggregated orders
            const nextExpected = calculateNextExpectedQty(order.customer_name, order.product_name, allOrders);
            
            html += `
                <tr>
                    <td class="text-muted small">${escapeHtml(order.time || '')}</td>
                    <td>${escapeHtml(order.customer_name)}</td>
                    <td>${escapeHtml(order.product_name)}</td>
                    <td class="text-center fw-bold">${order.quantity}</td>
                    <td class="text-center">
                        <span class="badge ${nextExpected > 0 ? 'bg-info' : 'bg-secondary'}">${nextExpected > 0 ? '~' + nextExpected : '-'}</span>
                    </td>
                </tr>
            `;
        });
    ...
    tbody.innerHTML = html || '<tr><td colspan="5" class="text-center text-muted">No orders found</td></tr>';
}

// Calculate next expected quantity based on customer's ordering pattern
function calculateNextExpectedQty(customerName, productName, allOrders) {
    // Filter orders for this customer and product
    ...
    const totalQty = relevantOrders.reduce((sum, o) => sum + parseInt(o.quantity || 0), 0);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const analysisTab = document.querySelector('[data-bs-target="#tab-analysis"]');
    if (analysisTab) {
        analysisTab.addEventListener('click', () => setTimeout(loadSalesAnalysis, 100));
        analysisTab.addEventListener('shown.bs.tab', loadSalesAnalysis);
    }
    
    document.getElementById('analysisApplyFilter')?.addEventListener('click', loadSalesAnalysis);
    document.getElementById('analysisDateRange')?.addEventListener('change', loadSalesAnalysis);
    document.getElementById('analysisCustomer')?.addEventListener('change', loadSalesAnalysis);

    // NEW: table-level filters for Daily Customer Orders
    document.getElementById('ordersApplyFilter')?.addEventListener('click', () => {
        if (window.salesAnalysisData) {
            renderDailyOrdersTable(window.salesAnalysisData, getOrdersTableFilters());
        } else {
            // if data not yet loaded, load then filters will be applied in loadSalesAnalysis
            loadSalesAnalysis();
        }
    });

    document.getElementById('ordersClearFilter')?.addEventListener('click', () => {
        const cf = document.getElementById('ordersCustomerFilter');
        const df = document.getElementById('ordersDateFrom');
        const dt = document.getElementById('ordersDateTo');
        if (cf) cf.value = '';
        if (df) df.value = '';
        if (dt) dt.value = '';
        if (window.salesAnalysisData) {
            renderDailyOrdersTable(window.salesAnalysisData, {});
        }
    });
});
